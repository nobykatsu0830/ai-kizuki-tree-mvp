#!/usr/bin/env python3
"""スペース別の「投稿パスワード（合言葉）」を設定する。

使い方:
    python3 set_space_join_password.py <space_slug> <password>   # 設定/変更
    python3 set_space_join_password.py <space_slug> ""           # 解除（誰でも投稿できる状態に戻す）

.env に DATABASE_URL があれば本番Postgres、無ければローカルSQLiteに対して実行する。
パスワードはSHA-256ハッシュで保存され、平文はDBに残らない。
未設定のスペースは従来どおり誰でも投稿できる（後方互換）。
"""
import os
import sys
...
def main() -> None:
    if len(sys.argv) < 3:
        print("usage: python3 set_space_join_password.py <space_slug> <password>")
        sys.exit(2)
    ...
    with pc.connect(pc.DEFAULT_DB_PATH) as conn:
        if not pc.space_exists(conn, slug):
            ...
            sys.exit(1)
        pc.set_space_join_password(conn, slug, password)
    print(f"スペース '{slug}' の投稿パスワードを{'解除しました（誰でも投稿可）' if not password else '設定しました'}。")
